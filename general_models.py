...
def log_normal(x, mu=None, v=None, reduce=True):
    ...
    logpdf = -0.5 * (torch.log(2 * np.pi) + v.log() + (x - mu).pow(2).div(v))
...
class inVAE(ABC):
    ...
    def load(
        ...
        self.module.load_state_dict(torch.load(save_dir))
    ...
    def setup_adata(
        ...
        **kwargs,
        ...
        """ encoders_hard_coded = {
            'obs': {
                'cell_type': lambda c: dict_encoders['cell_type'].transform(c.to_numpy()[:, None]),
                'batch': lambda c: dict_encoders['batch'].transform(c.to_numpy()[:, None])
            }
        }

        data_loader_hard = AnnLoader(adata, batch_size = batch_size, shuffle = True, use_cuda = use_cuda, convert = encoders_hard_coded)
        data_loader_hard.dataset[:].obs['cell_type']
        data_loader_hard.dataset[:].obs['batch'] """

        data_loader = AnnLoader(adata, batch_size = batch_size, shuffle = True, use_cuda = use_cuda, convert = encoders)
        #data_loader.dataset[:].obs['cell_type']
        #data_loader.dataset[:].obs['batch']

        #raise TypeError('Debug setup anndata...')
...
class FinVAE(inVAE):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dict_encoders, self.data_loading_encoders, self.data_loader, self.transformed_data = self.setup_adata(adata, batch_key, inv_covar_keys, spur_covar_keys, device, batch_size)
...
def debug_models(model_name = 'NFinVAE', decoder_dist = 'normal'):
    ...
    latent = model.get_latent_representation(adata_val, type='inv')
    print(f'The shape of the latents is: {latent.shape}')
